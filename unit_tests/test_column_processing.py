...
from preprocessing_functions.column_processing import normalizer, date_extraction, Imputation
...
def test_imputation(spark_session):
    ...
    assert imputed_df.filter(col("Zipcode").isNull() | col("value").isNull()).count() == 0
    ...
    assert imputed_df.select("Zipcode").where(col("Zipcode") == "99504").count() == 3
